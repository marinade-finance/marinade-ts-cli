...
async function show({
  ...
  let marinadeConfigParams: Partial<MarinadeConfig>
  if (stateAddress) {
    marinadeConfigParams = { marinadeStateAddress: stateAddress, connection }
  } else {
    marinadeConfigParams = { connection }
  }
  ...
  console.log(
    `Stake Delta Window: ${state.stakeSystem.slotsForStakeDelta.toString()} slots, ${
...
async function listValidatorsWithStake(marinadeState: MarinadeState) {
  ...
  console.log(
    `  Stake list account: ${state.stakeSystem.stakeList.account.toBase58()} with ${state.stakeSystem.stakeList.count}/${stakeCapacity} stakes`,
  )
  console.log('-----------------')
  ...
  console.log(
    `  List account: ${state.validatorSystem.validatorList.account.toBase58()} with ${state.validatorSystem.validatorList.count}/${validatorCapacity} validators`,
  )
  console.log('-------------------------------------------------------------')
    ...
    coolingDownStakes.forEach(stakeInfo => {
      ...
      if (delegation) {
        ...
        console.log(
          `  ${
            stakeInfo.index
          }. Stake ${stakeInfo.record.stakeAccount.toBase58()} (full balance ${stakeInfo.balance.toString()})`,
        )
      }
    })
  }
